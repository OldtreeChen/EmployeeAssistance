...
function post(path, body, jar) {
  return new Promise((resolve, reject) => {
    ...
    https.request({ hostname: HOST, path: '/ecp/' + path, method: 'POST',
      ...
      res.on('end', () => { const j = (res.headers['set-cookie']||[]).map(c=>c.split(';')[0]).join('; '); try{resolve({data:JSON.parse(raw),jar:j})}catch{resolve({data:raw,jar:j})} });
...
async function main() {
  ...
  // Test addMainUnitEntity without userId (session inference)
  const r1 = await post('Ecp.TimeReport.addMainUnitEntity.data', { actualWorktime: '0', actualWorkvalue: '0.0', date: '2026-04-23T00:00:00.000Z', couldSave: 1 }, jar);
  console.log('Without userId:', JSON.stringify(r1.data).substring(0, 300));

  // Test with empty userId
  const r2 = await post('Ecp.TimeReport.addMainUnitEntity.data', { userId: '', actualWorktime: '0', actualWorkvalue: '0.0', date: '2026-04-23T00:00:00.000Z', couldSave: 1 }, jar);
  console.log('Empty userId:', JSON.stringify(r2.data).substring(0, 300));
}
